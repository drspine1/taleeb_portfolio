      ...
      />
          ...
          <div  className={`absolute right-0 -bottom-5 ${id===5 && id===4 && "w-full opacity-0"
            ...
            "group-hover/bento:translate-x-2 transition duration-200 md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10 space-y-4 z-10 absolute top-0 left-0 right-0  bottom-0 "
              ...
              <div
          className={`font-sans font-extralight text-left max-w-[10rem] flex flex-wrap md:text-xs lg:text-base text-sm text-[#C1C2D3] z-10  ${id===3 && "max-w-[7rem] md:max-w-[10rem]"}`} >
          {description}
        </div>
          <div
          className={`font-sans text-xl lg:text-2xl max-w-96 font-bold z-10  text-white text-left ${id===3 && "max-w-90 md:max-w-96"}`} >
          {title}
        </div>
          {
            id===3 &&(
